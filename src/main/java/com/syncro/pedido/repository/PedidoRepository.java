 ...
 *
 * Los métodos adicionales usan dos estrategias: 
 * 1. Query Methods: Spring genera
 * el SQL automáticamente a partir del nombre del método (ej: findByEmpresa_Id →
 * SELECT * FROM pedido WHERE empresa_id = ?) 
 * 2. @Query con JPQL: consultas
 * personalizadas escritas manualmente cuando la lógica es más compleja que lo
 * que el nombre del método puede expresar.
 */
@Repository
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
     ...
     *
     * Spring traduce este nombre a: SELECT * FROM pedido WHERE empresa_id = ?
    ...
    /**
     * Busca todos los pedidos que están en un estado específico. Útil para
     * tareas administrativas o reportes internos.
     *
     * Spring traduce este nombre a: SELECT * FROM pedido WHERE estado = ?
     */
